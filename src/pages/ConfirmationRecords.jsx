...
export default function ConfirmationRecords() {
  ...
  async function handleDeleteVerified(reason) { setSaving(true); try { await archiveConfirmationRecord(selectedRecord.id, reason); setDeleteVerificationOpen(false); setSelectedRecord(null); await loadRecords({ showLoader: false }); showSnackbar('Confirmation record moved to Trash successfully.', 'success') } catch (error) { showSnackbar(error?.message || 'Failed to move to Trash: confirmation record.', 'error') } finally { setSaving(false) } }
