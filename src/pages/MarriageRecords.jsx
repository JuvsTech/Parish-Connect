...
export default function MarriageRecords() {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return matches.sort((a, b) => {
      const prefix = nameSort.startsWith('bride') ? 'bride' : 'groom'
      const keys = [prefix + 'LastName', prefix + 'FirstName', prefix + 'MiddleName', prefix + 'Suffix']
  ...
  async function handleDeleteVerified(reason) { setSaving(true); try { await archiveMarriageRecord(selectedRecord.id, reason); setDeleteVerificationOpen(false); setSelectedRecord(null); await loadRecords({ showLoader: false }); showSnackbar('Marriage record moved to Trash successfully.', 'success') } catch (error) { showSnackbar(error?.message || 'Failed to move to Trash: marriage record.', 'error') } finally { setSaving(false) } }
                ...
                <Box sx={{ p: 2 }}>
                  ...
                  <Stack spacing={1.75} divider={<Divider flexItem />}>
                    ...
                    <TextField select label="Sort by" size="small" fullWidth value={nameSort} onChange={event => setNameSort(event.target.value)}>
                      ...
                      <MenuItem value="groom-asc">Groom A?Z</MenuItem>
                      <MenuItem value="groom-desc">Groom Z?A</MenuItem>
                      <MenuItem value="bride-asc">Bride A?Z</MenuItem>
                      <MenuItem value="bride-desc">Bride Z?A</MenuItem>
                    </TextField>
                    <FilterSection
                      title="Minister"
