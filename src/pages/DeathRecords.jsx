...
export default function DeathRecords() {
  ...
  async function handleDeleteVerified(reason) { setSaving(true); try { await archiveDeathRecord(selectedRecord.id, reason); setDeleteVerificationOpen(false); setSelectedRecord(null); await loadRecords({ showLoader: false }); showSnackbar('Death record moved to Trash successfully.', 'success') } catch (error) { showSnackbar(error?.message || 'Failed to move to Trash: death record.', 'error') } finally { setSaving(false) } }
